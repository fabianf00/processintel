...
class TestInductiveMiningInfrequent(unittest.TestCase):
    ...
    def test_fallback_to_filtered_dfg(self):
        ...
        # Test that mining can handle very noisy log
        result = mining.inductive_mining(self.very_noisy_log)

        # Should produce a valid result even with noise
        self.assertIsNotNone(result)
    ...
    def test_edge_frequency_caching(self):
        """Test that generate_graph processes correctly."""
        mining = InductiveMiningInfrequent(self.noisy_log)
        ...
        # Should have generated a graph
        self.assertIsNotNone(mining.graph)
    ...
    def test_empty_and_edge_case_logs(self):
        ...
        # Empty log should return a loop structure or None from fallthrough
        self.assertIsNotNone(result)
    ...
    def test_performance_with_large_noisy_log(self):
        ...
        # Main patterns
        for i in range(20):
            large_noisy_log[("A", "B", "C", "D")] = (
                large_noisy_log.get(("A", "B", "C", "D"), 0) + 5
            )
            large_noisy_log[("A", "C", "B", "D")] = (
                large_noisy_log.get(("A", "C", "B", "D"), 0) + 4
            )
    ...
    @patch("app.mining_algorithms.inductive_mining_infrequent.get_logger")
