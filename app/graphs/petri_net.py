...
class PetriNetToolkit:
    ...
    def register_transition(
        self, transition_id: str, visible: bool, label: str | None = None
    ) -> None:
        net = self.get_current_net()
        transition = net["transitions"].setdefault(
            transition_id, {"inputs": set(), "outputs": set(), "visible": visible}
        )
    ...
    def finalize_net(self, net: dict | None = None) -> dict:
        ...
        for trans_id, data in net["transitions"].items():
            ...
            if not data["visible"]:
                ...
                has_no_visible_outputs = all(
                    place not in visible_input_places for place in outputs
                )
    ...
    def simulate_trace(
        self, trace: Sequence[str], start_events: set[str]
    ) -> tuple[int, bool]:
            ...
            for place_id in required_inputs:
                ...
                if marking.get(place_id, 0) == 0:
                    if not self._ensure_token(
                        place_id, transitions, marking, silent_to_place
                    ):
                        if is_start_input:
        ...
        remaining_tokens = {
            place: tokens for place, tokens in marking.items() if tokens > 0
        }
        ...
        final_ok = (
            True if not final_places else (final_tokens == 1 or not trace_sequence)
        )
        is_completed = (
            parsed_count == len(trace_sequence) and non_final_tokens == 0 and final_ok
        )
    ...
    def _fire_silent(
        ...
        while (
            transition_fired and cycles < max_cycles
        ):  # only continues if still firing
    ...
    for transition_id, data in petri_net["transitions"].items():
        ...
        if not data.get("visible", False):
            if not graph.contains_node(transition_id):
                ...
                graph.add_silent_transition(
                    transition_id, label=gw_label or "silent", data=gw_data, **gw_attrs
                )
            continue
    ...
    for place_id in petri_net["places"]:
        if not graph.contains_node(place_id):
            ...
            if gw_label is not None or gw_data or gw_attrs:
                graph.add_place(
                    place_id, label=gw_label or " ", data=gw_data, **gw_attrs
                )
            else:
...
def _safe_create_edge(
    graph, source: str, target: str, created_edges: set[tuple[str, str]], logger
) -> None:
    if graph.contains_node(source) and graph.contains_node(target):
        ...
        if logger:
            logger.debug(
                f"[PetriNetToolkit] Skipped edge {source} -> {target} (missing node)"
            )
